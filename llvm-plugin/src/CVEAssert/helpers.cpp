...
Function *getOrCreateResolveReportSantizerTriggered(Module &M) {
  ...
  Function *resolve_report_func = Function::Create(
      ...
      // FIXME: libresolve right now overrides this function and breaks exit
      // strategy Find a way to make libresolve respect CVE_ASSERT_STRATEGY and
      // then make this weak again
      (strcmp(CVE_ASSERT_STRATEGY, "EXIT") == 0) ? GlobalValue::InternalLinkage
